...
def load_checkpoint(checkpoint_path, device, mode='kd'):
    ...
    Args:
        ...
        mode: 'kd' (knowledge distillation), 'moco_v3' (contrastive learning), or 'vicreg'
    
    Returns:
        State dict for the encoder (student for KD, encoder_q for MoCo, encoder for VICReg)
    """
    try:
    ...
    if mode == 'moco_v3':
        ...
        # VICReg checkpoints contain 'model' with 'encoder' and 'proj'
        ...
        if 'model' in checkpoint:
            ...
            if encoder_state:
                ...
                # If no 'encoder.' prefix, assume the whole model is the encoder
...
def build_student_model(model_config, device, mode='kd'):
    """Build student model architecture"""
    if mode == 'vicreg':
        model_name = model_config.get('backbone_name', 'vit_small_patch16_224')
        img_size = model_config.get('image_size', 96)
    else:
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='kd', choices=['kd', 'moco_v3', 'vicreg'],
                       help='Training mode: kd (knowledge distillation), moco_v3 (contrastive learning), or vicreg')
    ...
    elif args.mode == 'vicreg':
        print("  Using encoder from VICReg checkpoints")
    else:
    ...
    student = build_student_model(model_config, device, mode=args.mode)
