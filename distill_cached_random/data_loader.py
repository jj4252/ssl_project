...
class RandomSubsetCachedDataset(Dataset):
    """
    Wrapper around CachedTensorDataset that samples a random subset each epoch.
    ...
    Key features:
    - Samples random indices from the full cached dataset each epoch
    - Configurable samples_per_epoch
    - Works seamlessly with max_steps_per_epoch
    - Resamples indices at the start of each epoch (via set_epoch)
    """
    def __init__(self, base_dataset: CachedTensorDataset, samples_per_epoch: int, seed: int = None):
        """
        Args:
            base_dataset: The full CachedTensorDataset to sample from
            samples_per_epoch: Number of random samples to use per epoch
            seed: Optional random seed (for reproducibility)
        """
        self.base_dataset = base_dataset
        self.full_size = len(base_dataset)
        self.samples_per_epoch = min(samples_per_epoch, self.full_size)
        self.seed = seed
        ...
        print(f"✓ RandomSubsetCachedDataset initialized:")
        print(f"  Full dataset size: {self.full_size:,} samples")
        print(f"  Samples per epoch: {self.samples_per_epoch:,} samples")
        print(f"  Coverage: {100 * self.samples_per_epoch / self.full_size:.1f}% per epoch")
    
    def _resample_indices(self):
        """Resample random indices for the current epoch"""
        ...
        self.current_indices = random.sample(range(self.full_size), self.samples_per_epoch)
        self.current_indices.sort()  # Sort for better cache locality
    
    def set_epoch(self, epoch: int):
        """
        Set the current epoch and resample indices.
    ...
    def __getitem__(self, idx: int) -> torch.Tensor:
        """
        Get item from the current epoch's random subset.
        
        Args:
            idx: Index in the current epoch's subset (0 to samples_per_epoch-1)
        
        Returns:
            Tensor from base_dataset at the randomly sampled index
        """
        if idx < 0 or idx >= self.samples_per_epoch:
            raise IndexError(f"Index {idx} out of range [0, {self.samples_per_epoch})")
        
        # Map subset index to full dataset index
        full_idx = self.current_indices[idx]
        return self.base_dataset[full_idx]
...
def build_pretraining_dataloader(data_config: dict, train_config: dict) -> torch.utils.data.DataLoader:
    ...
    if use_cached_tensors:
        ...
        # Check if random subset sampling is enabled
        ...
        if samples_per_epoch is not None and samples_per_epoch > 0:
            # Use random subset sampling
            dataset = RandomSubsetCachedDataset(
                base_dataset=base_dataset,
                samples_per_epoch=samples_per_epoch,
                seed=random_seed
            )
            print(f"✓ Using random subset cached dataset:")
            print(f"  Full cache: {len(base_dataset):,} samples")
            print(f"  Per epoch: {samples_per_epoch:,} random samples")
        else:
