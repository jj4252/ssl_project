"""
Optimized Knowledge Distillation Trainer for Fast Training
Designed to train in 20-40 minutes per epoch on A100/L4 GPUs

Key optimizations:
- Step-capped epochs (max_steps_per_epoch)
- Simplified augmentations (no local crops, reduced intensity)
...
def main():
    parser = argparse.ArgumentParser(description="Optimized Knowledge Distillation Training")
    ...
    # Dataset and DataLoader with optimized transforms
    use_multi_crop = train_cfg.get('use_multi_crop', False)
    use_local_crops = train_cfg.get('use_local_crops', False)
    ...
    if use_multi_crop:
        transform = FastMultiCropTransform(
            ...
            local_crops_number=data_cfg.get('local_crops_number', 0),
            image_size=student_img_size,
            use_local_crops=use_local_crops
        )
    else:
        # Simple single-crop transform (fastest)
    ...
    persistent_workers = train_cfg.get('persistent_workers', False)  # Must be False
    prefetch_factor = train_cfg.get('prefetch_factor', 2) if num_workers > 0 else None
    pin_memory = train_cfg.get('pin_memory', True)
    
    print(f"DataLoader settings: {num_workers} workers, persistent={persistent_workers}, "
