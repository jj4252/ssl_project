...
Modified for fixed shard subset (testing if random sampling degrades performance).
...
class FixedShardSubsetCachedDataset(Dataset):
    """
    Wrapper around CachedTensorDataset that uses a fixed set of shards.
    
    Instead of randomly sampling shards each epoch, this uses a fixed set of shard files
    (e.g., first 25 shards) for all epochs. This allows testing whether random sampling
    degrades performance compared to fixed shard selection.
    
    Key features:
    - Uses fixed shard files (not random)
    - Uses all images from selected shards sequentially
    - Better cache locality and I/O patterns
    - Same shards used across all epochs (no resampling)
    """
    def __init__(self, base_dataset: CachedTensorDataset, fixed_shards: int):
        """
        Args:
            base_dataset: The full CachedTensorDataset to sample from
            fixed_shards: Number of fixed shard files to use (takes first N shards)
        """
        self.base_dataset = base_dataset
        self.fixed_shards = fixed_shards
        
        # Get all available shards from base dataset
        self.all_shards = base_dataset.shards.copy()
        self.total_shards = len(self.all_shards)
        
        # Validate fixed_shards
        if fixed_shards > self.total_shards:
            print(f"⚠️  Warning: fixed_shards ({fixed_shards}) > total_shards ({self.total_shards}), using all shards")
            self.fixed_shards = self.total_shards
        
        # Select fixed shards (first N shards)
        self.current_shards = self.all_shards[:self.fixed_shards]
        
        # Build index mapping for fixed shard selection
        self._build_index_mapping()
        
        print(f"✓ FixedShardSubsetCachedDataset initialized:")
        print(f"  Total shards available: {self.total_shards}")
        print(f"  Fixed shards used: {self.fixed_shards}")
        print(f"  Samples per epoch: {self.num_samples:,}")
        print(f"  Coverage: {100 * self.fixed_shards / self.total_shards:.1f}% of shards")
    ...
    def set_epoch(self, epoch: int):
        """
        Set the current epoch (no-op for fixed shards, but kept for compatibility).
        """
        pass  # Fixed shards don't change across epochs
    
    def __getitem__(self, idx: int) -> torch.Tensor:
        """
        Get item from the fixed shard selection.
        
        Args:
            idx: Index in the fixed subset (0 to num_samples-1)
...
def build_pretraining_dataloader(data_config: dict, train_config: dict) -> torch.utils.data.DataLoader:
    ...
    if use_cached_tensors:
        ...
        # Check if fixed shard sampling is enabled (for testing if random sampling degrades performance)
        fixed_shards = data_config.get('fixed_shards', None)
        # Fallback to old samples_per_epoch for backward compatibility
        if fixed_shards is None:
            ...
            if samples_per_epoch is not None:
                ...
                fixed_shards = max(1, (samples_per_epoch + cache_shard_size - 1) // cache_shard_size)
                print(f"⚠️  Note: samples_per_epoch is deprecated, using fixed_shards={fixed_shards} (estimated)")
        
        if fixed_shards is not None and fixed_shards > 0:
            # Use fixed shard subset (for testing if random sampling degrades performance)
            dataset = FixedShardSubsetCachedDataset(
                base_dataset=base_dataset,
                fixed_shards=fixed_shards
            )
            print(f"✓ Using fixed shard subset cached dataset:")
    ...
    # If using FixedShardSubsetCachedDataset, we use fixed shards with sequential access,
    # so we can disable shuffle for better cache locality within shards
    should_shuffle = not isinstance(dataset, FixedShardSubsetCachedDataset)
